...
RESULTS_ROOT="cola_reliability/results/exp1_reasoning_0p5b_ft_vs_lora"
...
usage() {
  cat <<'EOF'
Usage:
  ./cola_reliability/run_exp1_reasoning_ft_vs_lora.sh
  ./cola_reliability/run_exp1_reasoning_ft_vs_lora.sh --quick
  sbatch cola_reliability/run_exp1_reasoning_ft_vs_lora.sh

Options:
  --quick                  Run 1 epoch on 128 train/eval examples per task
  --tasks CSV              Default: cola,mrpc,rte,sst2
  --model-name NAME        Default: Qwen/Qwen2.5-0.5B-Instruct
  --results-root DIR       Default: cola_reliability/results/exp1_reasoning_0p5b_ft_vs_lora
  --epochs N               Default: 5
  ...
  --attention-targets CSV  Default: auto. For Qwen auto resolves to q_proj,v_proj
...
if [[ $QUICK -eq 1 ]]; then
  ...
  TRAIN_SAMPLES=128
  EVAL_SAMPLES=128
  RESULTS_ROOT="cola_reliability/results/quick_exp1_reasoning_0p5b_ft_vs_lora"
fi
...
printf "task_name,method,model_name,epochs,batch_size,max_train_samples,max_eval_samples,lora_r,lora_alpha,lora_dropout,lora_placement,attention_targets,abstention_threshold,calibration_bins,seed,output_dir\n" > "$settings_csv"
{
  printf "| task_name | method | model_name | epochs | batch_size | max_train_samples | max_eval_samples | lora_r | lora_alpha | lora_dropout | lora_placement | attention_targets | abstention_threshold | calibration_bins | seed | output_dir |\n"
  printf "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n"
} > "$settings_md"
...
for task in "${TASKS[@]}"; do
  ...
  printf "%s,lora,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" "$task" "$MODEL_NAME" "$EPOCHS" "$BATCH_SIZE" "$TRAIN_SAMPLES" "$EVAL_SAMPLES" "$LORA_R" "$LORA_ALPHA" "$LORA_DROPOUT" "$LORA_PLACEMENT" "$ATTENTION_TARGETS" "$ABSTENTION_THRESHOLD" "$CALIBRATION_BINS" "$SEED" "$lora_dir" >> "$settings_csv"
  ...
  printf "| %s | lora | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n" "$task" "$MODEL_NAME" "$EPOCHS" "$BATCH_SIZE" "$TRAIN_SAMPLES" "$EVAL_SAMPLES" "$LORA_R" "$LORA_ALPHA" "$LORA_DROPOUT" "$LORA_PLACEMENT" "$ATTENTION_TARGETS" "$ABSTENTION_THRESHOLD" "$CALIBRATION_BINS" "$SEED" "$lora_dir" >> "$settings_md"

  echo "Reasoning 0.5B Exp1: ${task} full fine-tuning"
  ...
  echo "Reasoning 0.5B Exp1: ${task} LoRA"
  "$python_cmd" cola_reliability/run_cola_reliability.py \
    --task_name "$task" \
    --method lora \
    --output_dir "$lora_dir" \
...
echo "Done. Reasoning 0.5B Exp1 results written to $RESULTS_ROOT"
